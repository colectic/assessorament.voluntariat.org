...
myApp.controller("FormCtrl", function($http, $scope, Towns, Structures, Hows, Areas, CustomerCompany, CustomerUser, Ticket, InfyTicket){
  ...
  $scope.populate = function() {
    if ($scope.data.altres_dades.dni_nif.length == 9) {
        console.log('Find NIF and poulate fields');
        $scope.customer_company.$get({CIF: $scope.data.altres_dades.dni_nif}, function(data){
          console.log(data);
  ...
  $scope.send = function() {
    ...
    $scope.customer_company.$get({customer_id: customer_id}, function(data){ console.log('customer company found');
      ...
      $scope.ticket.$save(function(data){console.log('ticket created');}, function(error){console.log('error on ticket creation'); $scope.error = 'error on ticket creation';});
      $scope.infyticket.$save(function(data){console.log('infyticket created');}, function(error){console.log('error on infyticket creation'); $scope.error = 'error on infyticket creation';});
        ...
        $scope.ticket.$save(function(data){console.log('ticket created');}, function(error){console.log('error on ticket creation'); $scope.error = 'error on ticket creation';});
        $scope.infyticket.$save(function(data){console.log('infyticket created');}, function(error){console.log('error on infyticket creation'); $scope.error = 'error on infyticket creation';});
      }, function(error){ console.log('error on customer user creation'); $scope.error = 'error on customer user creation';
        });
      });
      ...
      }, function(error){ console.log('error on customer company creation'); $scope.error = 'error on customer company creation';
      });
    });
